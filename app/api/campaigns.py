...
from app.schemas.campaign import CampaignCreate, CampaignRead, MessageOut
...
def create_campaign(data: CampaignCreate, db: Session = Depends(get_db)):
    return cs.create_campaign(db, data)
...
@router.post("/{campaign_id}/recipients", status_code=status.HTTP_201_CREATED, response_model=list[RecipientRead])
def add_recipients(campaign_id: int, payload: RecipientsBulk, db: Session = Depends(get_db)):
    ...
    return rs.add_recipients(db, campaign_id, payload.items)
...
def list_recipients(campaign_id: int, db: Session = Depends(get_db)):
    ...
    return rs.get_recipients(db, campaign_id)
...
async def import_csv(
    ...
    return cs.import_csv(db, campaign_id, content)


@router.post("/{campaign_id}/start", status_code=status.HTTP_202_ACCEPTED, response_model=MessageOut)
def start_campaign(
    campaign_id: int,
    ...
    return MessageOut(detail="Рассылка запущена", campaign_id=camp.id)


@router.post("/{campaign_id}/stop", response_model=MessageOut)
def stop_campaign(campaign_id: int, db: Session = Depends(get_db)):
    camp = cs.get_campaign(db, campaign_id)
    cs.set_status(db, camp, CampaignStatus.PAUSED)
    return MessageOut(detail="Рассылка приостановлена", campaign_id=camp.id)
